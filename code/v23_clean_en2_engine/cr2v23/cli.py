...
from .engine import CleanEN2SelectedCI, EngineConfig
...
def cmd_run(args):
    cfg = EngineConfig(
        ...
        denom_floor=args.denom_floor,
        hybrid_residual_weight=args.hybrid_residual_weight,
        min_abs_residual=args.min_abs_residual,
        diagnostic_top_k=args.diagnostic_top_k,
    )
    eng = CleanEN2SelectedCI(cfg)
    result = eng.run()
    write_json(Path(args.output_dir) / "run_result.json", result)
    print(result)

def main(argv=None):
    p = argparse.ArgumentParser(prog="cr2v23")
    sub = p.add_subparsers(dest="cmd", required=True)

    b = sub.add_parser("build-integrals")
    b.add_argument("--output-dir", default="v23_cache")
    ...
    r.add_argument("--max-dim", type=int, default=120000)
    r.add_argument("--walltime-seconds", type=int, default=86400)
    r.add_argument("--add-per-iter", type=int, default=1000)
    r.add_argument("--max-iters", type=int, default=200)
    r.add_argument("--candidate-limit", type=int, default=853776)
    ...
    r.add_argument("--score-mode", default="en2", choices=["en2", "residual", "hybrid"])
    r.add_argument("--denom-floor", type=float, default=1e-8)
    r.add_argument("--hybrid-residual-weight", type=float, default=0.05)
    r.add_argument("--min-abs-residual", type=float, default=0.0)
    r.add_argument("--diagnostic-top-k", type=int, default=20)
